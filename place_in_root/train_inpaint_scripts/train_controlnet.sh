...
export OUTPUT_DIR="checkpoints_0310/SCALE_${CONDITIONING_SCALE}/depth.ckpt"
export HF_HOME="$SCRATCH_DIRECTORY/huggingface_cache"
export TRAIN_CODE_DIR="$WORKING_DIRECTORY/examples/controlnet/train_controlnet_inpaint_v3.py"
JSONL_FILE="metadata_lu_20000_320000.jsonl"
CHECKPOINT="$OUTPUT_DIR/checkpoint-4000"
...
echo "🎆 VKitti sampling"
python vkitti_sample_7-2-1-2.py \
  --depth_root "$DEPTH_ROOT" \
  --rgb_root "$RGB_ROOT" \
  --out_root "$OUT_ROOT" \
  --min_points 20000 --max_points 320000 \
  --points_sampling loguniform \
  --save_png --txt_name vkitti_lu_20000_320000.txt \
  --jsonl_name "$JSONL_FILE"
echo "🎇 VKitti sampling complete"


echo "🎆 Hypersim sample"
python examples/controlnet/hypersim_sample_7-2-1-2.py \
