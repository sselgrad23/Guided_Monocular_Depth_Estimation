...
def sample_diode_points(data_root, txt_out_base, num_points,
    ...
    txt_out = txt_out.with_name(f"{txt_out.stem}_N{num_points}{txt_out.suffix}")
    ...
    with open(txt_out_base, "r") as f:
        lines = [line.strip().split() for line in f.readlines()]
    depth_files = [data_root / l[1] for l in lines if len(l) >= 2]
    ...
    for npy_file in tqdm(depth_files, desc="Processing Diode", dynamic_ncols=True):
        ...
        rel_rgb = rgb_file.relative_to(data_root)
        rel_gt  = npy_file.relative_to(data_root)
        rel_sparse = sparse_path.relative_to(data_root)
        rel_mask   = mask_path.relative_to(data_root)
        entries.append(f"{rel_rgb} {rel_gt} {rel_mask} {rel_sparse}")
