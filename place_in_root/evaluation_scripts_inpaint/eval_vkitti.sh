...
export GT_DATASET="$SCRATCH_DIRECTORY/train_data/vkitti_eval"
export PRED_DIR="$CONTROLNET_DIR/0810/output_N_${N}_PROPER_CKPT_${CKPT}_SCALE_${SCALE}/vkitti_eval"
...
echo "🎆 Evaluation vkitti, ${N} points, Checkpoint ${CKPT}"
python eval_7-2-1-1.py \
     --file_list "$GT_DATASET/vkitti_${N}.txt" \
     --pred_root "$PRED_DIR" \
     --gt_root "$GT_DATASET" \
     ...
     --depth_min 1e-5 \
     --depth_max 80.0
echo "🎇 Evaluation vkitti, ${N} points, Checkpoint ${CKPT} complete"