...
export OUTPUT_DIR="$CONTROLNET_DIR/0810/output_N_${N}_PROPER_CKPT_${CKPT}_SCALE_${SCALE}/scannet"
...
echo "🎆 Inference Scannet, ${N} points, Checkpoint ${CKPT}"
python infer_inpaint.py \
     --pretrained_model_name_or_path "$MODEL_DIR" \
     --controlnet_model_name_or_path "$CHECKPOINT_DIR" \
     --resume_from_checkpoint "$CHECKPOINT_DIR_NO" \
     --split_file "$EVAL_DATASET/scannet_val_sampled_800_1_${N}.txt" \
     --data_root "$EVAL_DATASET/scannet_val_sampled_800_1" \
     ...
     --depth_min 1e-3 \
     --depth_max 10.0
echo "🎇 Inference Scannet, ${N} points, Checkpoint ${CKPT} complete"