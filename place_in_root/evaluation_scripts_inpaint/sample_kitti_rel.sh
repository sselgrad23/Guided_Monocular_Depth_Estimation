...
python sample_kitti_rel.py \
     --depth_root $EVAL_DATASET_1 \
     --rgb_root $EVAL_DATASET_1 \
     --txt_out $HOME_DIRECTORY/eval_dataset/kitti/eigen_val_from_train_800.txt \
     --num_points $N \
...
python sample_kitti_rel.py \
     ...
     --txt_out $HOME_DIRECTORY/eval_dataset/kitti/eigen_test_files_with_gt.txt \
     --num_points $N \
     --transform_order "sampling" \
     --seed 42
