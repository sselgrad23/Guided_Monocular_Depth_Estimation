...
export GT_DATASET="$SCRATCH_DIRECTORY/eval_dataset/nyuv2/nyu_labeled_extracted"
export PRED_DIR="$CONTROLNET_DIR/0810/output_N_${N}_PROPER_CKPT_${CKPT}_SCALE_${SCALE}/nyuv2"
...
echo "🎆 Evaluation NYUv2 train and test set, ${N} points, Checkpoint ${CKPT}"
python eval_7-2-1-1.py \
     --file_list "$GT_DATASET/nyuv2_${N}.txt" \
     --pred_root "$PRED_DIR" \
     --gt_root "$GT_DATASET" \
     ...
     --depth_min 1e-3 \
     --depth_max 10.0
echo "🎇 Evaluation NYUv2 train and test set, ${N} points, Checkpoint ${CKPT} complete"