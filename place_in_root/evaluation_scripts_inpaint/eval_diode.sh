...
export GT_DATASET="$SCRATCH_DIRECTORY/eval_dataset/diode/diode_val"
export PRED_DIR="$CONTROLNET_DIR/0810/output_N_${N}_PROPER_CKPT_${CKPT}_SCALE_${SCALE}/diode"
...
echo "🎆 Evaluation Diode, ${N} points, Checkpoint ${CKPT}"
python eval_7-2-1-1.py \
     --file_list "$GT_DATASET/diode_val_sampled_list_${N}.txt" \
     --pred_root "$PRED_DIR" \
     --gt_root "$GT_DATASET" \
     --output_dir "$PRED_DIR/eval_gt_as_pred_test" \
     --dataset_name "diode" \
     --depth_min 0.6 \
     --depth_max 350.0
echo "🎇 Evaluation Diode, ${N} points, Checkpoint ${CKPT} complete"