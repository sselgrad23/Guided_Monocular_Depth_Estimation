...
export CHECKPOINT_DIR="$CONTROLNET_DIR/checkpoints_0310/SCALE_${SCALE}/depth.ckpt"
export CHECKPOINT_DIR_NO="checkpoint-${CKPT}/controlnet"
export HF_HOME="$SCRATCH_DIRECTORY/huggingface_cache"
export GT_DATASET="$SCRATCH_DIRECTORY/eval_dataset/scannet"
export PRED_DIR="$CONTROLNET_DIR/0810/output_N_${N}_PROPER_CKPT_${CKPT}_SCALE_${SCALE}/scannet"
...
echo "🎆 Evaluation Scannet, ${N} points, Checkpoint ${CKPT}"
python eval_7-2-1-1.py \
     --file_list "$GT_DATASET/scannet_val_sampled_800_1_${N}.txt" \
     --pred_root "$PRED_DIR" \
     --gt_root "$GT_DATASET/scannet_val_sampled_800_1" \
     --output_dir "$PRED_DIR/eval" \
     --dataset_name "scannet" \
     --depth_min 1e-3 \
     --depth_max 10.0
echo "🎇 Evaluation Scannet, ${N} points, Checkpoint ${CKPT} complete"