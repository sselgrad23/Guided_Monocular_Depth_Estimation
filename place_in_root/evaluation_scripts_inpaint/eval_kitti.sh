...
export GT_DATASET="$SCRATCH_DIRECTORY/eval_dataset/kitti"
export PRED_DIR="$CONTROLNET_DIR/0810/output_N_${N}_PROPER_CKPT_${CKPT}_SCALE_${SCALE}/kitti"
...
echo "🎆 Evaluation kitti, ${N} points, Checkpoint ${CKPT}"
python eval_7-2-1-1.py \
     --file_list "$GT_DATASET/kitti_${N}.txt" \
     --pred_root "$PRED_DIR" \
     --gt_root "$GT_DATASET" \
     ...
     --depth_min 1e-5 \
     --depth_max 80.0
echo "🎇 Evaluation kitti, ${N} points, Checkpoint ${CKPT} complete"