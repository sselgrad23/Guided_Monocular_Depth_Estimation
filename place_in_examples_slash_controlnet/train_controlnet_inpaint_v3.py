...
"""
Training code for ControlNet + Stable Diffusion E2E FT with inpainting (no multiple steps)
Note: We change the ControlNet channels to accept 9 channels instead of 5. Extra loss function. 
"""
...
import accelerate
#from examples.controlnet.load_global import Hypersim, VirtualKITTI2
import numpy as np
import torch
...
def image_grid(imgs, rows, cols):
    # What is the type of imgs? # imgs is a list of PIL Image objects
    # What if I have numpy arrays instead of PIL Images? # If imgs is a list of numpy arrays, you can convert them to PIL Images using Image.fromarray() before passing them to this function.
...
def log_validation(
    ...
    if not is_final_validation:
        #print("🥼🥼🥼🥼🥼 THIS IS NOT THE FINAL VALIDATION")
    ...
    for validation_prompt, validation_image, validation_rgb_image in zip(validation_prompts, validation_images, validation_rgb_images):
        ...
        logger.info(f"🖼️ Saved validation image to {single_out_path}")
...
def binarize(image: torch.Tensor) -> torch.Tensor:
    """
    Return a 1-channel binary mask where 1 marks invalid/empty pixels
    (to repaint) and 0 marks valid pixels (to preserve).

    Args:
        image: [N,C,H,W] tensor, normalized to [-1, 1].

    Returns:
        mask: [N,1,H,W] float tensor in {0., 1.}
    """
    assert image.dim() == 4, f"Expected [N,C,H,W], got {tuple(image.shape)}"

    # If 3-channel, collapse to 1 channel (mean or pick the channel that carries depth)
    if image.size(1) > 1:
        image = image.mean(dim=1, keepdim=True)

    # Define holes:
    # - values <= -1.0 (common when zeros in [0,1] were mapped to -1)
    # - values > 1.0 (shouldn't happen after proper norm, but guarded)
    # - non-finite values (NaN/Inf)
    nonfinite = ~torch.isfinite(image)
    holes = (image <= -1.0) | (image > 1.0) | nonfinite

    # Convert to float mask (1.0 for holes/repaint, 0.0 for valid/preserve)
    mask = holes.to(dtype=image.dtype)
...
def valid_sparse_mask_from_control(control_img, eps=1e-6):
    # control_img: [N,3,H,W] in [-1,1] (stacked grayscale)
    # return [N,1,H,W] float mask: 1 at valid sparse pixels
    if control_img.size(1) > 1:
        depth1 = control_img[:, :1]  # any channel (they're identical)
    else:
        depth1 = control_img
    return (depth1 > (-1.0 + eps)).to(dtype=control_img.dtype)
...
def parse_args(input_args=None):
    ...
    parser.add_argument("--lambda_main", type=float, default=1.0, help="Weight for main e2e loss (SSI/MSE).")
    parser.add_argument("--lambda_sparse", type=float, default=1.0, help="Weight for sparse consistency loss.")
    parser.add_argument("--sparse_loss", type=str, default="huber", choices=["l1", "mse", "huber"],
                        help="Per-pixel loss on sparse points.")
    parser.add_argument(
        "--train_dataset_choice",
        type=str,
        default="mixed",
        choices=["mixed", "hypersim", "vkitti"],
        help="Choose which dataset(s) to train on: 'hypersim', 'vkitti', or 'mixed' (default: mixed 9:1).",
    )
...
def main(args):
    ...
    controlnet.train()
    # unet.train() # This is in the original diffusion-e2e-ft
    ...
    # Training datasets
    if args.train_dataset_choice == "hypersim":
        logger.info("Training only on Hypersim dataset")
        train_dataset = Hypersim(
            root_dir=args.hypersim_data_dir,
    ...
    elif args.train_dataset_choice == "vkitti":
        logger.info("Training only on Virtual KITTI 2 dataset")
        train_dataset = VirtualKITTI2(
            root_dir=args.vkitti_data_dir,
            transform=True,
            sparse_depth_name=args.vkitti_sparse_depth_name
        )
        train_dataloader = torch.utils.data.DataLoader(
            train_dataset,
            shuffle=True,
        ...
        logger.info("Training on mixed dataset (9:1 Hypersim:VKITTI)")
        train_dataset_hypersim = Hypersim(
            root_dir=args.hypersim_data_dir,
            csv_filepath=args.hypersim_csv_filepath,
            transform=True
        )
        train_dataset_vkitti = VirtualKITTI2(
            root_dir=args.vkitti_data_dir,
            transform=True,
            sparse_depth_name=args.vkitti_sparse_depth_name
        )
        train_dataloader_vkitti = torch.utils.data.DataLoader(
            train_dataset_vkitti, shuffle=True, batch_size=args.train_batch_size, num_workers=args.dataloader_num_workers
        )
        train_dataloader_hypersim = torch.utils.data.DataLoader(
            train_dataset_hypersim, shuffle=True, batch_size=args.train_batch_size, num_workers=args.dataloader_num_workers
        )
    ...
    if args.train_dataset_choice == "mixed":
        total_examples = len(train_dataset_hypersim) + len(train_dataset_vkitti)
    elif args.train_dataset_choice == "hypersim":
        total_examples = len(train_dataset)
    else:
        total_examples = len(train_dataset)
    
    logger.info("***** Running training *****")
    logger.info(f"  Num examples = {total_examples}")
    ...
    for epoch in range(first_epoch, args.num_train_epochs):
        ...
        for step, batch in enumerate(train_dataloader):
            with accelerator.accumulate(controlnet):
                ...
                control_latents = encode_image(vae, batch["sparse_depth"].to(device=accelerator.device, dtype=weight_dtype))
                control_latents = control_latents * vae.config.scaling_factor

                # Build binary mask from control image (pixel space), then resize to latent resolution
                mask = binarize(controlnet_image)  # [N,1,H,W]
                mask = F.interpolate(mask, size=control_latents.shape[2:], mode="nearest")  # [N,1,h,w]
                ...
                controlnet_input = torch.cat([rgb_latents, control_latents, mask], dim=1).to(accelerator.device)

                down_block_res_samples, mid_block_res_sample = controlnet(
                    unet_input,
                    timesteps,
                    encoder_hidden_states=encoder_hidden_states,
                    controlnet_cond=controlnet_input, # I would have the latent sparse depth for inpainting
                    conditioning_scale=args.conditioning_scale, # Could try changing this to see the effects
                    return_dict=False,
                )
                ...
                if val_mask.any():
                    ...
                    decoded = decode_image(vae, current_latent_estimate)

                    # Post-process predicted images and retrieve ground truth for SSI loss
                    current_estimate = decoded.mean(dim=1, keepdim=True) 
                    ...
                    if torch.isnan(estimation_loss_ssi).any():
                        estimation_loss_ssi = torch.tensor(0.0, device=accelerator.device, dtype=weight_dtype)
                    
                    # valid_sparse_mask_from_control
                    valid_sparse = valid_sparse_mask_from_control(controlnet_image)  # [N,1,H,W]
                    pred_dec1    = decoded[:, :1]                                    # first grayscale channel
                    ctrl_dec1    = controlnet_image[:, :1]
                    
                    if args.sparse_loss == "l1":
                        perpx = torch.abs(pred_dec1 - ctrl_dec1)
                    elif args.sparse_loss == "mse":
                        perpx = (pred_dec1 - ctrl_dec1) ** 2
                    else:  # huber
                        perpx = F.huber_loss(pred_dec1, ctrl_dec1, reduction="none")

                    #loss_main = estimation_loss_ssi   # already masked, etc.
                    loss_sparse = (perpx * valid_sparse).sum() / valid_sparse.sum().clamp_min(1.0)

                    loss = args.lambda_main * estimation_loss_ssi + args.lambda_sparse * loss_sparse
                                        # control (sparse input)
        ...
        # Log it and print with emojis
        # accelerator.log({"final_epoch_loss": final_epoch_loss}, step=global_step)
        # log the same scalar key you used per step
    ...
    if accelerator.is_main_process:
        #logs = {"loss": loss.detach().item(), "lr": lr_scheduler.get_last_lr()[0], "train_loss": train_loss}
