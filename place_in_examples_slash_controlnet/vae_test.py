...
def main():
    ...
    # Data loading (normalisation also takes place here)
    ...
    # Give an optional limit on the number of images to process
    ...
    # Loader's epsilon used in depth normalization
    EPS = 0.02
    ...
    for bidx, batch in enumerate(dl):
        # Depth is already normalized to [-1,1] and stacked to 3 channels
        x_sparse = batch["depth"].to(device, dtype=torch.float32)  # Bx3xHxW
        B = x_sparse.size(0)

        # Valid mask (invalid sparse → exactly -1 after normalization)
        ...
        # Metrics on valid depth pixels only
        ...
        for i in range(B):
            ...
            # Place visualised versions side by side with before and after on the left and right respectively
            if args.side_by_side:
                cmp_png = outdir / "viz" / f"sparse_cmp_{i_global:06d}.png"
            ...
            # Delete npy files to save space, if the user doesn't choose to keep them
            ...
            rows.append({
                ...
                "sparse_mae": mae[i],
                "sparse_psnr": psnr[i],
                "sparse_ssim": (ssim_vals[i] if HAS_SSIM else None),
    ...
    print("\n=== VAE RECON (SPARSE DEPTH, normalized, masked by valid points) ===")
    ...
    # --- write per-image CSV ---
    if rows:
        csv_path = outdir / "per_image_metrics_sparse.csv"
