...
# Do they have a StableDiffusion pipeline? Like something with that name? 
class MarigoldPipeline(DiffusionPipeline):
    ...
    def __call__(
        ...
        # Why are we repeating the input image?
        # Because we want to perform ensembling, where we generate multiple predictions
        # from the same input image to improve the robustness and accuracy of the depth/normal estimation.
...
# Do the outputs stay scaled in the range of [0, 1]? # Yes, the outputs are scaled to the range of [0, 1] for depth maps and [-1, 1] for normal vectors.
    ...
    def single_infer(
        ...
        # When would there be already an empty text embedding?
        # If the empty text embedding is not set, we encode it.
        # This is typically done once at the beginning of the pipeline.
        # It is used to condition the UNet during the denoising process.
        # If the empty text embedding is already set, we can reuse it.
        # This is useful for efficiency, as we don't need to encode it again.
