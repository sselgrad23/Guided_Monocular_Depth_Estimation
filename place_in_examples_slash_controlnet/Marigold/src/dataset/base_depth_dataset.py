...
class BaseDepthDataset(Dataset):
    ...
    def _get_data_item(self, index):
        rgb_rel_path, depth_rel_path, mask_rel_path, sparse_rel_path, filled_rel_path = self._get_data_path(index)

        rasters = {}
        rasters.update(self._load_rgb_data(rgb_rel_path=rgb_rel_path))

        # load depth if not RGB_ONLY (for GT)
        if DatasetMode.RGB_ONLY != self.mode:
            depth_data = self._load_depth_data(depth_rel_path, filled_rel_path)
        ...
        # load sparse depth in *all* modes
        if sparse_rel_path is not None:
            sparse_data = self._load_sparse_depth_data(sparse_rel_path)
            rasters.update(sparse_data)
    ...
    def _load_rgb_data(self, rgb_rel_path):
        ...
        outputs = {
            "rgb_int": torch.from_numpy(rgb).int() / 255.0,
        }
        return outputs
    ...
    # New function for loading sparse depth
    def _load_sparse_depth_data(self, sparse_rel_path):
        sparse_depth = self._read_depth_file(sparse_rel_path).squeeze()
        sparse_depth_linear = torch.from_numpy(sparse_depth).float().unsqueeze(0)
        return {"sparse_depth_linear": sparse_depth_linear.clone()}
    ...
    def _get_data_path(self, index):
        ...
        depth_rel_path = mask_rel_path = sparse_rel_path = filled_rel_path = None

        # every mode (even RGB_ONLY) should now load sparse
        if len(filename_line) >= 2:
            depth_rel_path = filename_line[1]
        if len(filename_line) >= 3:
            mask_rel_path = filename_line[2]
        if len(filename_line) >= 4:
            sparse_rel_path = filename_line[3]
        if self.has_filled_depth and len(filename_line) >= 5:
            filled_rel_path = filename_line[4]

        return rgb_rel_path, depth_rel_path, mask_rel_path, sparse_rel_path, filled_rel_path
