...
if "__main__" == __name__:
    ...
    for data in tqdm(dataloader, desc="Evaluating"):
        ...
        for met_func in metric_funcs:
            ...
            # Use repr to avoid any implicit rounding in string conversion
            sample_metric.append(repr(_metric))
    ...
    # Use repr for summary values as well to avoid rounding by tabulate's default float formatting
    _results = metric_tracker.result()
    _headers = list(_results.keys())
    _values = [repr(v) for v in _results.values()]
    eval_text += tabulate([_headers, _values], tablefmt="github", disable_numparse=True)
    ...
    csv_numbers_path = os.path.join(output_dir, "eval_metrics_numbers_only.csv")
