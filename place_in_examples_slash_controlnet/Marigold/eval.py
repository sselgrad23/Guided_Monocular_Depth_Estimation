...
if "__main__" == __name__:
    ...
    # --- Also save CSV summaries (like in eval_7-2-1-1.py) ---
    stats = metric_tracker.result()  # dict: {metric_name: mean_value}

    # 1) Named CSV: "Metric,Mean"
    csv_path = os.path.join(output_dir, "eval_metrics.csv")
    with open(csv_path, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        writer.writerow(["Metric", "Mean"])
        for name, mean_val in stats.items():
            # use str/ repr to avoid scientific formatting surprises, like your custom script
            writer.writerow([name, repr(mean_val)])

    # 2) Numbers-only CSV (single row of means in the same metric order)
    csv_numbers_path = os.path.join(output_dir, "num_only_eval_metrics.csv")
    with open(csv_numbers_path, "w", newline="") as csvfile:
        writer = csv.writer(csvfile)
        # preserve order from stats (Python 3.7+ dict preserves insertion order)
