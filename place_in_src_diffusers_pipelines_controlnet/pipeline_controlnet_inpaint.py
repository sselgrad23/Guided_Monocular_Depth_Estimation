...
class StableDiffusionControlNetInpaintPipeline(
    ...
    def __init__(
        ...
        self.controlnet = self.edit_controlnet_for_latent_input(self.controlnet)
        ...
        # We don't use the below but we keep it for now
    ...
    @staticmethod
    def retrieve_latents_marigold(encoder_output):
    ...
    def prepare_latents_marigold(
        ...
        batch_size: int,
    ) -> Tuple[torch.Tensor, torch.Tensor]: # TODO: ensemble_size: int, batch_size: int,

        image_latent = torch.cat(
            [
                self.retrieve_latents_marigold(self.vae.encode(rgb_image[i : i + batch_size]))
                for i in range(0, rgb_image.shape[0], batch_size)
            ],
            dim=0,
        )  # [N,4,h,w]
        image_latent = image_latent * self.vae.config.scaling_factor # [N*E,4,h,w]
        # TODO: image_latent = image_latent.repeat_interleave(ensemble_size, dim=0)  # [N*E,4,h,w]

        # add
        # provide zeros as noised latent
        pred_latent = self.zeros_tensor(
            image_latent.shape,
            device=image_latent.device,
            dtype=image_latent.dtype,
        )  # [N*E,4,h,w]
        #print(f"prepare_latents_marigold Image latent shape: {image_latent.shape}, Pred latent shape: {pred_latent.shape}")
        """pred_latent = latents
        if pred_latent is None:
            ...
            pred_latent *= self.scheduler.init_noise_sigma"""
    ...
    def prepare_latents_marigold_control(
        self,
        control_image: torch.Tensor,
        batch_size: int,
    ) -> Tuple[torch.Tensor, torch.Tensor]: # TODO: ensemble_size: int, batch_size: int,
        """prepare latents for control image, we assume control image is already downsampled using self.image_processor.preprocess"""

        image_latent = torch.cat(
            [
                self.retrieve_latents_marigold(self.vae.encode(control_image[i : i + batch_size]))
                for i in range(0, control_image.shape[0], batch_size)
            ],
            dim=0,
        )  # [N,4,h,w]
        image_latent = image_latent * self.vae.config.scaling_factor # [N*E,4,h,w]
        ...
        mask = self.binarize(control_image)  # [N,1,H,W]
        mask = F.interpolate(mask, size=image_latent.shape[2:], mode="nearest")  # [N,1,h,w]
    ...
    def decode_prediction(self, pred_latent: torch.Tensor) -> torch.Tensor:
        ...
        denom = (prediction.max() - prediction.min()).clamp_min(1e-8)
        prediction = (prediction - prediction.min()) / denom
    ...
    def __call__(
        ...
        strength: float = 1.0,
        num_inference_steps: Optional[int] = None, #int = 50,
        guidance_scale: float = 1,
        negative_prompt: Optional[Union[str, List[str]]] = None,
        num_images_per_prompt: Optional[int] = 1,
        eta: float = 0.0,
        generator: Optional[Union[torch.Generator, List[torch.Generator]]] = None,
        latents: Optional[Union[torch.Tensor, List[torch.Tensor]]] = None, #Optional[torch.Tensor] = None,
        prompt_embeds: Optional[torch.Tensor] = None,
        negative_prompt_embeds: Optional[torch.Tensor] = None,
        ip_adapter_image: Optional[PipelineImageInput] = None,
        ip_adapter_image_embeds: Optional[List[torch.Tensor]] = None,
        output_type: Optional[str] = "pt",  #Optional[str] = "pil",
        return_dict: bool = True,
        cross_attention_kwargs: Optional[Dict[str, Any]] = None,
        controlnet_conditioning_scale: Union[float, List[float]] = 1.0, #0.5,
        ...
        batch_size: int = 1, # TODO: ensembling_kwargs: Optional[Dict[str, Any]] = None,
        ...
        **kwargs,
        ...
        #og_control = control_image.clone().to(device=device, dtype=dtype)
        ...
        control_image, _, _ = self.image_processor.preprocess( # control_image, control_padding, control_original_resolution
            control_image, processing_resolution, resample_method_input, device, dtype
        )  # [N,3,PPH,PPW]
        ...
        rgb_image_latent, pred_latent = self.prepare_latents_marigold(
            rgb_image, batch_size #TODO: , ensemble_size
        )  # [N*E,4,h,w], [N*E,4,h,w]
        control_image_latent, mask = self.prepare_latents_marigold_control(
            control_image, batch_size
        )  # [N*E,4,h,w], [N*E,4,h,w]

        #print(f"Shapes: control_image_latent {control_image_latent.shape}, mask {mask.shape}")
        #pdb.set_trace()
        controlnet_image_input = torch.cat([rgb_image_latent, control_image_latent, mask], dim=1)  # [N*E,9,h,w]
        ...
        for i in self.progress_bar(
            ...
            batch_image_latent = rgb_image_latent[i : i + batch_size]  # [B,4,h,w]
            ...
            for t in self.progress_bar(self.scheduler.timesteps, leave=False, desc="Diffusion steps..."):
                ...
                down_block_res_samples, mid_block_res_sample = self.controlnet(
                    batch_latent, #control_model_input,
                    t,
                    encoder_hidden_states=controlnet_prompt_embeds,
                    controlnet_cond=controlnet_image_input, # control_image, #image,
                ...
                batch_pred_latent = self.scheduler.step(
                    noise_pred, t, batch_pred_latent, **extra_step_kwargs
                ).prev_sample
        ...
        #print(f"prediction shape after decoding: {prediction.shape}, dtype: {prediction.dtype}, device: {prediction.device}")
        ...
        if match_input_resolution:
            ...
            #print(f"prediction shape after resizing to original resolution {original_resolution}: {prediction.shape}, dtype: {prediction.dtype}, device: {prediction.device}")
            #print(f"og_control size: {og_control.size()}, dtype: {og_control.dtype}, device: {og_control.device}")
            #prediction = torch.where(og_control != 0, og_control, prediction) # Test just to see how this would affect metrics

        
        #print(f"prediction type after processing: {type(prediction)}")
        ...
        if not return_dict:
            #print("Returning prediction and pred_latent as tuple")
